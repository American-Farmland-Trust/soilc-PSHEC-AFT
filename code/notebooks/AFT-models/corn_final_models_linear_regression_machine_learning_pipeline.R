...
#  MN_MI_WI = c('MN','MI','WI'),
#  KY_MO_TN = c('KY','MO','TN'),
#  MD_NY_NJ_PA = c('MD','NY','NJ','PA'),
#  NC_SC_WV_VA = c('NC','SC','WV','VA'),
#  KS_ND_NE_OK_SD = c('KS','ND','NE','OK','SD'),
#  AL_LA_MS_TX = c('AL','LA','MS','TX'))
...
state_id <- list(
  group_23 = c('MN','MI','WI','KY','MO','TN','MD','NY','NJ','PA','NC','SC','WV','VA','KS','ND','NE','OK','SD','AL','LA','MS','TX'),
  IA_IL_IN_OH = c('IA','IL','IN','OH')
  )
...
for (i in names(state_id)) {
  ...
  #--------individual state----------------#
  set.seed(1501)
  corn_split <- initial_split(data)
  corn_train <- training(corn_split)
  corn_test  <- testing(corn_split)
  
  set.seed(1502)
  corn_folds <- 
    vfold_cv(corn_train, repeats = 5)
  ...
  basic_rec <- 
    recipe(Yield_decomp_add ~ 
             ...
             ssurgo_cec_mean +ssurgo_h +  GEOID + state_alpha, data =corn_train) %>%
...
if (i == 'all') {
  ...
  ggsave(model_rmse, filename = paste0('data/soil/Model_selection/corn/', i , '_ML_model_selection.png'))
  # show metrics
  ...
  ggsave(rf_model_plot, filename = paste0('data/soil/Model_selection/corn/', i , '_ML_model_obs.vs.pred.png'))
  ...
  write.csv(rmse_rf, file = paste0('data/soil/Model_selection/corn/', i , '_ML_model_obs.vs.pred_county_mean_rmse.csv'))
  ...
  ggsave(rf_county_mean_plot, filename = paste0('data/soil/Model_selection/corn/', i , '_ML_model_obs.vs.pred_county_mean.png'))
  ...
  ggsave(pdp_plot_rf, filename = paste0('data/soil/Model_selection/corn/',i, '_ML_model_pdp_profile.png'))
