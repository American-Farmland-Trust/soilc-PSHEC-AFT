#' ---
#' title: "soy grouped_states_model_pipeline.Rmd"
#' author: "ML"
#' date: "2023-06-05"
...
all_data <- readRDS('data/AFT-data/soy_all_data_n15_om10_AFT.rds')

rainfed <- all_data %>%
  filter(irrigated == 'rainfed') 

levels(as.factor(rainfed$state_alpha))
#[1] "AL" "GA" "IA" "IL" "IN" "KS" "KY" "LA" "MD" "MI" "MN" "MO" "MS" "NC" "ND" "NE" "NJ" "NY" "OH" "OK" "PA" "SC" "SD" "TN" "TX" "VA" "WI" "WV"

# create a list of state (/group of state) names

state_id <- list(
  group_23 = c("AL", "GA","KS", "KY", "LA", "MD", "MI", "MN", "MO", "MS", "NC", "ND", "NE", "NJ", "NY","OK", "PA", "SC", "SD", "TN", "TX", "VA", "WI", "WV"),
  IA_IL_IN_OH = c('IA','IL','IN','OH')
)
...
for (i in names(state_id)) {
  ...
  #--------individual state----------------#
  set.seed(1501)
  soy_split <- initial_split(data)
  soy_train <- training(soy_split)
  soy_test  <- testing(soy_split)
  
  set.seed(1502)
  soy_folds <- 
    vfold_cv(soy_train, repeats = 5)
  ...
  basic_rec <- 
    recipe(Yield_decomp_add ~ 
             ...
             ssurgo_cec_mean +ssurgo_h +  GEOID + state_alpha, data =soy_train) %>%
    ...
    all_workflows <- all_workflows %>%
      ...
      update_workflow_model(id = 'gam_om_gen_additive_mod_10', spec = gam_model,
                            formula = Yield_decomp_add ~ s(ssurgo_om_mean) + 
                              ...
                              state_alpha_IA + state_alpha_IL + state_alpha_IN + state_alpha_KS +        
                              state_alpha_KY + state_alpha_LA + state_alpha_MD + state_alpha_MI + state_alpha_MN +        
                              state_alpha_MO + state_alpha_MS + state_alpha_NC + state_alpha_ND + state_alpha_NE + state_alpha_NJ +        
                              state_alpha_NY + state_alpha_OH + state_alpha_OK + state_alpha_PA + state_alpha_SC +      
                              state_alpha_SD + state_alpha_TN + state_alpha_TX + state_alpha_VA + state_alpha_WI +        
                              state_alpha_WV + state_alpha_GA) %>%
      update_workflow_model(id = 'gam_gen_additive_mod_11', spec = gam_model,
                            formula = Yield_decomp_add ~ s(ssurgo_om_mean) + 
                              ...
                              state_alpha_NY + 
                              state_alpha_OH + 
                              state_alpha_OK + state_alpha_PA + state_alpha_SC +      
                              state_alpha_SD + state_alpha_TN + state_alpha_TX + state_alpha_VA + state_alpha_WI +        
                              state_alpha_WV + state_alpha_GA)
    ...
    all_workflows <- all_workflows %>%
      ...
      update_workflow_model(id = 'gam_om_gen_additive_mod_10', spec = gam_model,
                            formula = Yield_decomp_add ~ s(ssurgo_om_mean) + 
                              ...
                              ssurgo_awc_mean + ssurgo_aws_mean + ssurgo_fifteenbar_mean +
                              ssurgo_cec_mean +ssurgo_h + 
                              #state_alpha_IA + state_alpha_IL + state_alpha_IN + 
                              state_alpha_KS + state_alpha_KY + state_alpha_LA + state_alpha_MD + state_alpha_MI + state_alpha_MN +        
                              state_alpha_MO + state_alpha_MS + state_alpha_NC + state_alpha_ND + state_alpha_NE + state_alpha_NJ +        
                              state_alpha_NY + 
                              #state_alpha_OH + 
                              state_alpha_OK + state_alpha_PA + state_alpha_SC +      
    ...
    all_workflows <- all_workflows %>%
      ...
      update_workflow_model(id = 'gam_om_gen_additive_mod_10', spec = gam_model,
                            formula = Yield_decomp_add ~ s(ssurgo_om_mean) + 
                              ...
                              ssurgo_awc_mean + ssurgo_aws_mean + ssurgo_fifteenbar_mean +
                              ssurgo_cec_mean +ssurgo_h + 
                              state_alpha_IL + state_alpha_IN + state_alpha_OH) %>% #IA will not be in the dummy variables
      update_workflow_model(id = 'gam_gen_additive_mod_11', spec = gam_model,
                            formula = Yield_decomp_add ~ s(ssurgo_om_mean) + 
                              s(ssurgo_clay_mean) + s(ssurgo_silt_mean) + s(ssurgo_sand_mean) + 
                              s(ssurgo_awc_mean) + s(ssurgo_aws_mean) + s(ssurgo_fifteenbar_mean) +
                              s(ssurgo_cec_mean) + s(ssurgo_h) +
                              state_alpha_IL + state_alpha_IN + state_alpha_OH)
  ...
  ggsave(model_rmse, filename = paste0('data/soil/Model_selection/soy/', i , '_ML_model_selection.png'))
  # show metrics
  ...
  ggsave(rf_model_plot, filename = paste0('data/soil/Model_selection/soy/', i , '_ML_model_obs.vs.pred.png'))
  ...
  saveRDS(final_wf, file = paste0('data/soil/Model_selection/soy/', i , '_final_workflow.rds'))
  
  model_final <- fit(final_wf, data)
  
  saveRDS(model_final, file = paste0('data/soil/Model_selection/soy/', i , '_final_model.rds'))
  ...
  write.csv(rmse_rf, file = paste0('data/soil/Model_selection/soy/', i , '_ML_model_obs.vs.pred_county_mean_rmse.csv'))
  ...
  ggsave(rf_county_mean_plot, filename = paste0('data/soil/Model_selection/soy/', i , '_ML_model_obs.vs.pred_county_mean.png'))
  ...
  ggsave(pdp_plot_rf, filename = paste0('data/soil/Model_selection/soy/',i, '_ML_model_pdp_profile.png'))
  ...
  # since there is no built-in functions to generate prediction intervals for randomforest/boosting trees, 
  ...
  write.csv(all_om_test_pred_int, file = paste0('data/soil/Coefficients/soy/', i , '_ML_model_OM_range_yield_predictions_intervals.csv'))
  ...
  #  select(GEOID,ssurgo_om_mean,ssurgo_om_county_mean, pred_yield, conf_lower, conf_upper) %>%
  #  left_join(meta_data, by = c('GEOID','state_alpha'))
  
  #write.csv(all_om_test_conf_int, file = paste0('data/soil/Coefficients/soy/', i , '_ML_model_OM_range_yield_confidence_intervals.csv'))
