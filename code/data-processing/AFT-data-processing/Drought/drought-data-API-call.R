...
#ML: keep just the rainfed counties
corn <- read_rds("data/AFT-data/corn_yield_2000-2022_w_irrigation_n_15.rds")
corn_r <- corn %>% filter(irrigated =='rainfed')
counties <- c(unique(corn_r$GEOID))

#Chellie's line
#counties <- c(unique(read_rds("data/AFT-data/corn_yield_2000-2022_w_irrigation_n_15.rds")$GEOID))
...
# Save raw data
save(list = c("data_by_county_2"), file = "data/AFT-data/Drought-data/drought_by_county_current.RData")
